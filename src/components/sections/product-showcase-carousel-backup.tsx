...
export function ProductShowcase({ translations: initialTranslations }: { translations: Translation['es'] }) {
  ...
  const openVideo = (vimeoId: string) => {
    ...
    // Timeout de seguridad: ocultar loading después de 3 segundos
    setTimeout(() => {
              ...
              {openVideoModal && (
                <>
                  <iframe
                        ...
                        if (typeof window !== 'undefined' && (window as any).Vimeo) {
                          try {
                            ...
                            player.on('error', (error: any) => {
                              console.error('Error al cargar video de Vimeo:', error);
                              setLoadingVideo(false);
                            });
                          } catch (error) {
                            console.error('Error inicializando Vimeo Player:', error);
                            setLoadingVideo(false);
                          }
                        }
                      }
                    ...
                    onLoad={() => {
                      // Fallback: si el iframe carga pero el player no responde
